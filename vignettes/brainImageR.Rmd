...
"developing" or "adult". 
- `reps` : The final significance estimates are determined by a
bootstrapping procedure. Increasing the number of iterations or "reps"
...
with `boot` using a one-tailed test depending on whether 
the value represents an enrichment or a depletion.  
`boot` only requires the output of `SpatialEnrichment` to run.
